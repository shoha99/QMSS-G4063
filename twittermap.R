# I include @simplystats twitter map function
# so it'll be so easy to just establish your twitter app authentication, then one line command to produce the map.

#establish twitter connection
#you would need ur secret code (those are mine, it wont work at yours! get yours from https://dev.twitter.com/apps/)
cred <- OAuthFactory$new(consumerKey="6skgOUKz6S2m6j3JjTww", 
                         consumerSecret="F9J8riuUlkAKA0NVai1twbnLULpWykGkfxD7K1S8K78",
...
myTwitteruserName <- getUser('shoha99')

#one line code to get the map!
twitterMap("NLTTC","New York","NLTTCTwitterMap.pdf",200,"both")
...
twitterMap <- function(userName,userLocation=NULL,fileName="twitterMap.pdf",nMax = 1000,plotType=c("followers","both","following")){
  ...
  if(plotType=="both"){
    ...
    mtext("Created by @shoha99",side=1,adj=1,line=0,cex=0.8,col="grey")
    mtext("Credit to @simplystats",side=1,adj=1,line=1,cex=0.8,col="grey")
    dev.off()
  }
